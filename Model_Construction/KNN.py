...
for method_name, method in feature_selection_methods.items():
    ...
    plt.savefig(f"auc_curve_KNN_{method_name}.pdf")  # 保存 AUC 图
    plt.close()
    ...
    # 保存最优模型
    joblib.dump(best_model, f'best_knn_model_{method_name}.pkl')
    print(f"使用 {method_name} 方法的最优模型已保存到 best_knn_model_{method_name}.pkl 文件中。")
    ...
    for i in range(1, 9):
        ...
        X_other = X_other[train_cols]  # 使用与训练数据一致的特征顺序
        ...
        if group_0_count >= 5 and group_1_count >= 5:
            ...
            plt.xlabel('时间 (月)')
            plt.ylabel('生存概率')
            plt.savefig(f"./外部数据{i}/批次处理/survival_curve_KNN_{method_name}.pdf")
            plt.close()
        else:
            print(
        ...
        df_other.to_excel(f"./外部数据{i}/批次处理/other_with_predictions_KNN_{method_name}.xlsx", index=False)
        print(
            f"df_other 的预测结果已保存到 ./外部数据{i}/批次处理/other_with_predictions_KNN_{method_name}.xlsx 文件中。")
    print("-" * 50)