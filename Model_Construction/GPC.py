...
def preprocess_data(df):
    ...
    y = df['Group'] if 'Group' in df.columns else None  # 如果没有 'Group' 列，则 y 为 None
    return X, y
...
feature_selection_methods = {
    ...
    'RFE_LR': RFE(estimator=LogisticRegression(max_iter=1000), n_features_to_select=10),
    'SelectFromModel_RF': SelectFromModel(RandomForestClassifier(n_estimators=500, random_state=42)),
    'SelectFromModel_LR': SelectFromModel(LogisticRegression(penalty='l1', solver='liblinear', max_iter=1000)),
    'RFE_RF': RFE(estimator=RandomForestClassifier(n_estimators=500, random_state=42), n_features_to_select=10),
}
...
for method_name, method in feature_selection_methods.items():
    ...
    # 高斯过程模型参数优化和交叉验证
    param_grid = {
        'kernel': [ConstantKernel(1.0) * RBF(length_scale=1.0, length_scale_bounds=(1e-5, 1e8)),
                   ConstantKernel(1.0) * RBF(length_scale=0.5, length_scale_bounds=(1e-5, 1e8)),
                   ConstantKernel(1.0) * RBF(length_scale=2.0, length_scale_bounds=(1e-5, 1e8))],
    ...
    grid_search = GridSearchCV(model, param_grid, scoring='roc_auc', cv=cv, n_jobs=6)  # n_jobs 根据 CPU 核心数调整
    ...
    plt.savefig(f"auc_curve_GP_{method_name}.pdf")
    plt.close()
    ...
    # 保存最优模型
    joblib.dump(best_model, f'best_gp_model_{method_name}.pkl')
    print(f"使用 {method_name} 方法的最优模型已保存到 best_gp_model_{method_name}.pkl 文件中。")
    ...
    for i in range(1, 9):
        ...
        X_other_selected = method.transform(X_other)
        # 特征缩放
        X_other_scaled = scaler.transform(X_other_selected)  # 对外部数据进行特征缩放
        df_other['Prediction'] = best_model.predict(X_other_scaled)
        ...
        # 检查每个组的样本数量，如果少于 5 个样本，则不绘制生存曲线并打印提示信息
        ...
        if group_0_count < 5 or group_1_count < 5:
            print(f"外部数据 {i} 的分组样本数不足 (Non-cluster3: {group_0_count}, Cluster3: {group_1_count})，无法绘制生存曲线。")
            continue  # 跳过当前循环，继续下一个外部数据
        else:
            ...
            plt.xlabel('时间 (月)')
            plt.ylabel('生存概率')
            plt.savefig(f"./外部数据{i}/批次处理/survival_curve_GP_{method_name}.pdf")
            plt.close()

        # 11. 保存 df_other 的预测结果
        df_other.to_excel(f"./外部数据{i}/批次处理/other_with_predictions_GP_{method_name}.xlsx", index=False)
        print(
            f"df_other 的预测结果已保存到 ./外部数据{i}/批次处理/other_with_predictions_GP_{method_name}.xlsx 文件中。")
    print("-" * 50)