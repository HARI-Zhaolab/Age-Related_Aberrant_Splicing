...
for method_name, method in feature_selection_methods.items():
    ...
    param_grid = {
        'penalty': ['l1', 'l2'],  # 正则化类型
        'C': np.logspace(-4, 4, 20),  # 正则化强度
        'solver': ['liblinear', 'saga']  # 优化器
    }

    cv = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
    model = LogisticRegression(max_iter=1000, random_state=42)  # 初始化逻辑回归模型
    ...
    plt.savefig(f"auc_curve_LR_{method_name}.pdf")  # 保存 AUC 图
    plt.close()
    # 打印验证集的 AUC 值 (也就是绘图展示的 AUC)
    print(f"验证集 AUC 得分: {roc_auc:.2f}")

    # 保存最优模型
    joblib.dump(best_model, f'best_lr_model_{method_name}.pkl')
    print(f"使用 {method_name} 方法的最优模型已保存到 best_lr_model_{method_name}.pkl 文件中。")
    ...
    for i in range(1, 7):
        ...
        if group_0_count >= 5 and group_1_count >= 5:
            ...
            plt.xlabel('时间 (月)')
            plt.ylabel('生存概率')
            plt.savefig(f"./外部数据{i}/批次处理/survival_curve_LR_{method_name}.pdf")
            plt.close()
        else:
            print(
        ...
        df_other.to_excel(f"./外部数据{i}/批次处理/other_with_predictions_LR_{method_name}.xlsx", index=False)
        print(f"df_other 的预测结果已保存到 ./外部数据{i}/批次处理/other_with_predictions_LR_{method_name}.xlsx 文件中。")
    print("-" * 50)