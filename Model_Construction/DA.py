...
for method_name, method in feature_selection_methods.items():
    ...
    # LDA 模型参数优化和交叉验证
    param_grid = {
        'solver': ['lsqr', 'eigen'],  # 只使用支持 shrinkage 的求解器
        'shrinkage': ['auto'] + list(np.linspace(0.1, 1, num=10)),  # 调整 shrinkage 范围
        'tol': [1e-4, 1e-3, 1e-2]  # 尝试不同的容差值
    }

    cv = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
    model = LinearDiscriminantAnalysis()
    grid_search = GridSearchCV(model, param_grid, scoring='roc_auc', cv=cv, n_jobs=5)
    ...
    plt.savefig(f"auc_curve_LDA_{method_name}.pdf")  # 保存 AUC 图
    plt.close()  # 关闭当前图像，避免重复绘制
    #打印验证集的 AUC 值 (也就是绘图展示的 AUC)
    print(f"验证集 AUC 得分: {roc_auc:.2f}")
    # 保存最优模型
    joblib.dump(best_model, f'best_lda_model_{method_name}.pkl')
    print(f"使用 {method_name} 方法的最优模型已保存到 best_lda_model_{method_name}.pkl 文件中。")
    ...
    for i in range(1, 9):
        ...
        if group_0_count >= 5 and group_1_count >= 5:
            ...
            plt.xlabel('时间 (月)')
            plt.ylabel('生存概率')
            plt.savefig(f"./外部数据{i}/批次处理/survival_curve_DA_{method_name}.pdf")
            plt.close()
        else:
            print(
        ...
        df_other.to_excel(f"./外部数据{i}/批次处理/other_with_predictions_LDA_{method_name}.xlsx", index=False)
        print(
            f"df_other 的预测结果已保存到 ./外部数据{i}/批次处理/other_with_predictions_LDA_{method_name}.xlsx 文件中。")
    print("-" * 50)